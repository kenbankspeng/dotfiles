...
local function log(mode, filepath)
  ...
  if file then
    ...
    for _, keymap in ipairs(keymaps) do
      ...
      if key_length > max_key_length then max_key_length = key_length end
      if command_length > max_command_length then max_command_length = command_length end
    end
    ...
    local function create_filler(length) return string.rep(' ', length) end
