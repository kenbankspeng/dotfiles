...
local function reset_root()
  if project_root then vim.cmd('Neotree ' .. project_root) end
end
...
local function parent(state)
  ...
  vim.defer_fn(function() require('neo-tree.ui.renderer').focus_node(state, parent_id) end, 40) -- ensure navigate up is finished
end
...
local binary_mime_types = {
  'application/octet-stream',
  -- Add more binary MIME types here if needed
}

local function is_previewable(filepath)
  local handle = io.popen('file --mime-type -b ' .. vim.fn.shellescape(filepath))
  if not handle then return false end
  local result = handle:read('*a')
  handle:close()
  for _, mime_type in ipairs(binary_mime_types) do
    if result:find(mime_type, 1, true) then return false end
  end
  return true
end

local function open_preview_buffer(filepath, state)
  ...
  -- Set the Neo-tree window width to 30% of the total columns
  local total_cols = vim.o.columns
  local neo_tree_width = math.floor(total_cols * 0.3)

  -- Focus on the Neo-tree window and resize it
...
local function preview_file(state)
  ...
  if not require('neo-tree.utils').is_expandable(node) then
    ...
    if filepath ~= nil then
      ...
      open_preview_buffer(filepath, state)
...
return {
  {
    ...
    keys = {
      { '..', file_manager.neotree_left, 'toggle neotree left' },
